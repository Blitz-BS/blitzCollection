...
Les créances ont leur montant initial indique dans `amount` de l'écriture, ce montant est soit **le véritable montant initial de la créance et le restant dû est calculer à partir des `paiement`et `avoir`** ou **le montant restant de la créance si l'historique n'est pas transmissible**.
...
* Une référence interne du dossier. Cette référence unique est celle du dossier dans le système se connectant.
...
Les fichiers d'exemples permettent d'avoir une idée de la représentation des différents objets. En particulier le fichier [`invoices.folders.exemple.json`](https://github.com/Blitz-BS/blitzCollection/blob/main/examples/invoices.folders.example.json) est un exemple de création de dossiers avec leur factures.

Le format de transfert complet se fait par **cabinet de recouvrement** via le schema [`root.schema.json`](https://github.com/Blitz-BS/blitzCollection/blob/main/examples/root.schema.json) dont un exemple est trouvable ici : [`root.example.json`](https://github.com/Blitz-BS/blitzCollection/blob/main/examples/root.example.json)